...
bool ShadowTest::prepare(const vkb::ApplicationOptions &options)
{
...
void ShadowTest::prepare_pipelines()
{
...
void ShadowTest::build_command_buffers()
{
...
void ShadowTest::render(float delta_time)
{
	if (!prepared)
	{
		return;
...
std::unique_ptr<vkb::VulkanSample> create_shadow_test()
{
	return std::make_unique<ShadowTest>();
}
